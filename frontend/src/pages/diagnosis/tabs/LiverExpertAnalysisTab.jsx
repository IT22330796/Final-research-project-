...
function LiverExpertAnalysisTab({ aiAssessment }) {
        ...
        return (
            <div className="flex flex-col items-center justify-center p-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-500 text-center max-w-sm mt-2">
                    The specialized medical visual analysis is still being processed or unavailable for this scan.
                </p>
            </div>
        )
    }
    ...
    const findSection = (keywordsList) => {
        // First try finding in split sections
        let section = sections.find(s => 
            keywordsList.some(k => s.replace(/[*#]/g, '').toLowerCase().includes(k.toLowerCase()))
        ) || '';
        
        // If not found in sections, try extracting from the full text using regex
        if (!section) {
    ...
    const keywords = ['Identified Symptoms & Signs', 'Detected Visual Patterns', 'Recommended Next Steps', 'Risk Level', 'Suggested Specialist'];
    ...
    const checkup = findSection(['Suggested Specialist', 'Specialist'])
    ...
    return (
        <div className="h-full overflow-y-auto pr-2 space-y-6 pb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-amber-50 rounded-2xl border-2 border-amber-200 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-5 flex-1">
                        <List spacing={2}>
                            {symptomsPoints.length > 0 ? symptomsPoints.map((item, i) => (
                            ...
                            )) : <p className="text-xs text-amber-700 italic">No specific symptoms identified in the report.</p>}
                        </List>
                    </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-5 flex-1">
                        <div className="space-y-4">
                            {patternsPoints.length > 0 ? patternsPoints.map((item, i) => (
                            ...
                            )) : <p className="text-xs text-slate-700 italic">No patterns detected in the current scan.</p>}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="p-5 flex-1 space-y-4">
                        <div className="flex items-center justify-between p-3 bg-white rounded-xl border border-rose-200 shadow-sm">
                            ...
                            <span className={`px-4 py-1 rounded-full text-xs font-black uppercase ${risk.toLowerCase().includes('high') ? 'bg-rose-600 text-white' :
                                ...
                                {risk.replace(/Risk Level/i, '').replace(':', '').replace(/[*#]/g, '').trim() || 'Moderate'}
                            </span>
                        </div>

                        <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-100 shadow-sm mb-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-bold text-indigo-900">
                                    {checkup.replace(/Suggested Specialist/i, '').replace(':', '').replace('•', '').replace(/[*#]/g, '').trim() || 'Hepatologist'}
                                </span>
                            </div>
                        </div>
                        ...
                        <Button 
                            ...
                            fullWidth
                            onClick={handleOpen}
                            className="bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 rounded-xl shadow-lg transform transition active:scale-95"
                            startIcon={<span>🧬</span>}
                        >
                ...
                <div className="bg-blue-50 rounded-2xl border-2 border-blue-200 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-5 flex-1">
                        <div className="flex flex-wrap gap-2">
                            ...
                            )) : <p className="text-xs text-blue-700 italic">No next steps provided.</p>}
                        </div>
                    </div>
                </div>
            </div>
            ...
            {(symptomsPoints.length === 0 || patternsPoints.length === 0) && (
                <div className="bg-gray-100/50 p-6 rounded-2xl border border-gray-200 italic">
                    <Typography variant="body2" className="text-gray-500 text-xs mb-2">RAW REPORT DEBUG VIEW:</Typography>
                    <pre className="text-[10px] whitespace-pre-wrap text-gray-400">{aiAssessment.validation_report}</pre>
                </div>
            )}
            ...
            <LiverRiskPredictor open={open} handleClose={handleClose} />
        </div>
    )
}
